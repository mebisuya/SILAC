...
COLOR_VECTOR_SAMPLE_TIME <- c('human_0.5' = '#FFE6BC',
                              'human_1' = '#FFE4B6',
                              'human_1.5' = '#FFDEA4',
                              'human_2' = '#FFD488',
                              'human_3' = '#FFC763',
                              ...
                              'mouse_6' = '#6D9CC3',
                              'mouse_12' = 'steelblue',
                              'mouse_2DG_0.5' = '#EBDBF9',
                              'mouse_2DG_1' = '#E9D8F8',
                              'mouse_2DG_1.5' = '#E5D0F7',
                              'mouse_2DG_2' = '#DFC5F5',
                              'mouse_2DG_3' = '#D6B6F3',
                              'mouse_2DG_6' = '#CCA2F0',
                              'mouse_2DG_12' = '#c08ced')
...
```
## Filtering of data (remove contaminants, filter for qupm)
...
Will only be used for PCA and MA plots
...
Number of unique gene names in each replicate
...
FILENAME <- "FigureS_Unique_gene_names_each_sample"

df_plot <- df %>% 
  dplyr::select(Sample, Replicate, time, gene_name) %>% 
  mutate(fill = paste0(Sample, "_", time)) %>% 
  mutate(x = factor(paste0(Sample, " -      ", Replicate, " - ", time, "h"),
                    levels = c("human -      Rep1 - 0.5h",
                               "human -      Rep1 - 1h",
                               "human -      Rep1 - 1.5h",
                               "human -      Rep1 - 2h",
                               "human -      Rep1 - 3h",
...
ggplot(df_plot,
  ...
  xlab(label = "") +
  ylab(label = bquote(~ "unique gene names")) +
  theme_HH_scatter_paper +
  scale_fill_manual(values = COLOR_VECTOR_SAMPLE_TIME) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0, hjust = 1)) +
  guides(fill = FALSE) +
  ggtitle("Number of unique gene names per sample\nbefore filtering for SILAC quant")
...
## PCA of vsn-corrected values

Use median of value for each gene name
...
FILENAME <- "FigureS_PCA_total_intensity"

## Data for PCA
df_temp <- df %>% 
  mutate(log_vsn = log2(vsn_norm_TotalSignalSum)) %>% 
  group_by(gene_name, source_file) %>% 
  summarise(log_vsn = median(log_vsn)) %>% 
  pivot_wider(names_from = source_file,
              values_from = log_vsn)
  
df_PCA <- t(na.omit(df_temp))

colnames(df_PCA) <- df_PCA[1,]

df_PCA <- df_PCA[-1,]

df_PCA_m <- mapply(df_PCA, FUN = as.numeric)
df_PCA_m <- matrix(data = df_PCA_m, ncol = ncol(df_PCA), nrow = nrow(df_PCA))

## Remove any infinite values
df_PCA_m[df_PCA_m == -Inf] <- NA
df_PCA_m <- t(na.omit(t(df_PCA_m)))

## Modifying
raw_PCA <- prcomp(df_PCA_m, scale = FALSE)
...
annotations_temp <-
  tibble(source_file = rownames(df_PCA)) %>% 
  left_join(metadata %>% 
              dplyr::select(source_file, Sample, time, Replicate))

PCA_raw_data <-
  data.frame(PC1 = raw_PCA$x[, 1],
             PC2 = raw_PCA$x[, 2],
             Sample = annotations_temp$Sample,
             time = annotations_temp$time,
             Replicate = annotations_temp$Replicate) %>% 
  mutate(fill = paste0(Sample, "_", time))

ggplot(data = PCA_raw_data, aes(PC1, PC2)) +
  geom_point(aes(fill = fill, shape = Replicate),
             size = 4,
             color = "black") +
  theme_HH_scatter_paper +
  scale_fill_manual(values = COLOR_VECTOR_SAMPLE_TIME) +
  scale_shape_manual(values=c("Rep1" = 22,
                              "Rep2" = 21,
                              "Rep3" = 23)) +
  guides(size = FALSE,
         fill = FALSE) +
  ggtitle("Vsn-corrected total Intensity (H+L)") +
...
write.xlsx2(PCA_raw_data %>% 
              dplyr::select(PC1, PC2, Sample, time, Replicate) %>% 
           ungroup() %>%
           as.data.frame(),
         file = paste0(FILENAME, "_", VERSION, ".xlsx"),
         sheet = "PCA",
...
rm(df_temp, annotations_temp, df_PCA, df_PCA_m, df_temp_w, PCA_raw_data, raw_PCA)

```

## Correlation plot across all samples
...
FILENAME <- "FigureS_pairwise_correlation"


df_corr_plot <- df %>% 
  mutate(log_vsn = log2(vsn_norm_TotalSignalSum),
         key = paste0(Sample, "_", time, "_", Replicate)) %>% 
...
cor_color <- viridis::viridis(n = 200, alpha = 1)

m_cor <- cor(df_corr_plot, use = "pairwise.complete.obs")

pdf(paste0(FILENAME, "_", VERSION, ".pdf"),
       width = FIG_WIDTH,
       height = FIG_HEIGHT)

corrplot::corrplot(m_cor, order = "hclust", col = rev(cor_color), mar=c(0,0,1,0),
                   title = "correlation of vsn-normalized total intensity (H+L)",
                   type = "full",
                   tl.col = "black",
                   tl.cex = 0.5,
                   is.corr = FALSE
...
## Filtering of data (remove cases without SILAC ratios as at least one SILAC channel is missing)

```{r filter for SILAC ratios}
